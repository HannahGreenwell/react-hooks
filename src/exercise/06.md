...
- can't directly use async/await in useEffect as the hook can only return a
  cleanup function (async/await automatically returns a promise)
- React will generally batch simultaneous state updates to avoid multiple
  re-renders
- state updates can not be batched in an asynchronous callback (using a state
  object or useReducer can be used instead)
- error boundary to catch request/runtime errors
- throw error in function component
- use key prop to reset (unmount/mount) error boundary
